...
def main():
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        with open(args.output_dir + 'val_predicts_big5.jsonl', 'w') as file1:
            for row in predicts:
                # file.write(f'{row}\n')
                json.dump(row, file1)
                file1.write(f'\n')

        with open(args.output_dir + 'val_predicts_utf-8.jsonl', 'w') as file2:
            for row in predicts:
                file2.write(f'{row}\n')
                # json.dump(row, file2)
                # file2.write(f'\n')
        ...
        rouge_score = json.loads(subprocess.check_output(f'python3.8 eval.py -r hw3_data/public.jsonl -s {args.output_dir}val_predicts_big5.jsonl 2> /dev/null', shell=True).decode("utf-8"))
