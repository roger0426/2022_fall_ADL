...
def parse_args():
    ...
    parser.add_argument( "--num_beam_groups", type=int, default=None,)
...
def main():
    ...
    validation_idx_list = [item['id'] for item in raw_datasets['test']]
        ...
        with torch.no_grad():
            ...
            for i, item in enumerate(decoded_preds):
                predicts.append({"title": item.replace("<extra_id_0>",  ""), \
                                    "id": validation_idx_list[args.per_device_test_batch_size*step+i]})
                    
        
    with open(args.output_file, 'w') as file1:
        for row in predicts:
            # file.write(f'{row}\n')
            json.dump(row, file1)
            file1.write(f'\n')
    
    with open(args.output_dir + 'predicts_utf8.jsonl', 'w') as file2:
        for row in predicts:
            file2.write(f'{row}\n')
            # json.dump(row, file2)
            # file2.write(f'\n')
